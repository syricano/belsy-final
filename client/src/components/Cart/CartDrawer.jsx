      ...
      >
            ...
            cart.items.map((item) => (
              <div key={item.id} className="bg-[var(--b1)] text-[var(--bc)] p-3 rounded-lg border border-[var(--border-color)] space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <ActionButton type="delete" label="Remove" onClick={() => removeItem(item.id)} />
                </div>
                <p className="text-sm opacity-70">${(item.priceGross ?? item.price ?? 0).toFixed(2)} each (incl. VAT)</p>
              </div>
            ))
          )}
        </div>
        ...
        <div className="p-4 border-t border-[var(--border-color)] space-y-3">
          ...
          <div className="flex gap-2">
            <button className="btn btn-outline flex-1" onClick={handleViewCart} disabled={!cart.items?.length}>View Cart</button>
            <button className="btn btn-primary flex-1" onClick={handleCheckout} disabled={!cart.items?.length}>Checkout</button>
          </div>
        </div>
      </div>
    </>
