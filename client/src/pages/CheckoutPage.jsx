...
const CheckoutPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await clear();
      toast.success(t('checkout.place_order'));
      reload();
    ...
    return (
      <section className="main-section min-h-screen flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <button className="btn btn-primary" onClick={() => navigate('/menu')}>{t('checkout.browse_menu')}</button>
        </div>
      </section>
    );
  }
    ...
    <section className="main-section min-h-screen py-10 px-4">
      <div className="max-w-5xl mx-auto grid gap-8 lg:grid-cols-2">
        <div className="bg-[var(--n)] text-[var(--nc)] p-6 rounded-2xl shadow-lg border border-[var(--border-color)]">
          <h2 className="text-2xl font-serif font-semibold mb-4">{t('checkout.order_summary')}</h2>
          <div className="space-y-3">
            {cart.items.map((item) => (
              <div key={item.id} className="flex justify-between">
                <div>
                  <p className="font-semibold">{item.name}</p>
                  <p className="text-sm opacity-70">{t('checkout.qty')}: {item.quantity} • ${(item.priceGross ?? item.price ?? 0).toFixed(2)} {t('cart.each_with_vat')}</p>
                </div>
        ...
        <div className="bg-[var(--n)] text-[var(--nc)] p-6 rounded-2xl shadow-lg border border-[var(--border-color)]">
          <h2 className="text-2xl font-serif font-semibold mb-4">{t('checkout.title')}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { label: t('checkout.name'), name: 'name', type: 'text', required: true },
              { label: t('checkout.email'), name: 'email', type: 'email', required: true },
              { label: t('checkout.phone'), name: 'phone', type: 'text', required: true },
            ].map((field) => (
            ...
            <div className="space-y-1">
              <label className="block text-sm font-medium">{t('checkout.note')}</label>
              <textarea
                name="note"
                rows="3"
                value={form.note}
